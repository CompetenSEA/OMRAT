...
- **Stateless API façade** (`api/workbench_api.py`) for load/import/sync/preview/run.
- **Stateful controller** (`api/workbench_controller.py`) with task queue semantics to mimic QGIS background task UX.
...
- **Map preview helpers** (`frontend/.../model/mapPreview.js`) for SVG-friendly route/object rendering from canonical payloads.
...
3. Build React tab components using reducer + mapPreview helpers and connect to polling endpoints.
